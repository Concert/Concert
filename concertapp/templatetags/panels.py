...
###
#   Parser for the Panel tag.
#   Usage: {% panel 'logged_in/partials/list_panel.html' 'list_panel' %}
###
def get_panel(parser, token):
    try:
        tag_name, template_location, id = token.split_contents()
    except ValueError:
    ...
    return PanelNode(template_location[1:-1], id[1:-1])
...
class PanelNode(template.Node):
    def __init__(self, template_location, id):
        self.template_location = template_location
        self.id = id
    def render(self, context):
        return render_to_string(self.template_location, {'id': self.id}, context)
